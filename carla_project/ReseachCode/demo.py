...
# Zone Class for tracking violations
class Zone:
    def __init__(self, name, start_x, start_y, end_x, end_y, id):
        self.name = name
    ...
    def is_in_zone(self, x, y):
        return (self.start_x <= x <= self.end_x or self.end_x <= x <= self.start_x) and                (self.start_y <= y <= self.end_y or self.end_y <= y <= self.start_y)

    def check_zone_entry_exit(self, x, y, screen, font):
        in_zone = self.is_in_zone(x, y)

        if in_zone and not self.is_user_in_zone:
            # User entered the zone
            self.violations += 1
            self.is_user_in_zone = True
            self.display_text(screen, font, f"Entering Zone{self.id}")
            pygame.display.flip()  # Refresh the screen
            pygame.time.wait(1000)  # Wait 1 second to make sure the text is visible

        elif not in_zone and self.is_user_in_zone:
            # User left the zone
            self.is_user_in_zone = False
            self.display_text(screen, font, f"Leaving Zone{self.id}")
            pygame.display.flip()  # Refresh the screen
            pygame.time.wait(1000)  # Wait 1 second to make sure the text is visible

    def display_text(self, screen, font, message):
        text = font.render(message, True, (0, 0, 0))  
        screen.blit(text, (screen.get_width() // 2 - text.get_width() // 2, 20))  # Top middle

# Initialize zones
zones = [
    Zone("Top Right Curve Zone", -10.5, -197.2, -77.0, -147.2, 1),
    Zone("Top Middle Straight Zone", -77.0, -147.2, -78.8, 143.6, 2),
    Zone("Top Left Curve Zone", -78.8, 143.6, -25.6, 202.3, 3),
    Zone("Middle Right Straight Zone", 162.5, -194.4, -10.5, -197.2, 4),
    Zone("Bottom Right Curve Zone", 236.3, -62.7, 162.5, -194.4, 5),
    Zone("Bottom Middle Straight Zone", 235.2, 138.8, 236.3, -62.7, 6),
    Zone("Bottom Left Curve Zone", 171.4, 200.2, 235.2, 138.8, 7),
    Zone("Middle Left Straight Zone", -25.6, 202.3, 171.4, 200.2, 8)
]

def check_user_position(x, y, screen):
    font = pygame.font.Font(None, 36)  # Font size 36
    for zone in zones:
        zone.check_zone_entry_exit(x, y, screen, font)

# Add this to your existing loop where player position is updated
def update_player_position(player, screen):
    # Get player coordinates
    transform = player.get_transform()
    location = transform.location
    check_user_position(location.x, location.y, screen)
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            update_player_position(world.player, display)

            # Your existing rendering and logic updates
            pygame.display.flip()
            ...
            # Track and update speed during the trial (velocity to mph)
            v = world.player.get_velocity()
            speed = 2.23694 * math.sqrt(v.x**2 + v.y**2 + v.z**2)  # Convert m/s to mph
            trial_manager.track_speed(speed, world.player)
...
# Create zones based on your coordinates
zones = [
    Zone("Top Right Curve Zone", -10.5, -197.2, -77.0, -147.2),
    Zone("Top Middle Straight Zone", -77.0, -147.2, -78.8, 143.6),
    Zone("Top Left Curve Zone", -78.8, 143.6, -25.6, 202.3),
    Zone("Middle Right Straight Zone", 162.5, -194.4, -10.5, -197.2),
    Zone("Bottom Right Curve Zone", 236.3, -62.7, 162.5, -194.4),
    Zone("Bottom Middle Straight Zone", 235.2, 138.8, 236.3, -62.7),
    Zone("Bottom Left Curve Zone", 171.4, 200.2, 235.2, 138.8),
    Zone("Middle Left Straight Zone", -25.6, 202.3, 171.4, 200.2)
]

def check_user_position(x, y):
    for zone in zones:
        if zone.is_in_zone(x, y):
            zone.record_violation()
...
class Zone:
    ...
    def is_in_zone(self, x, y):
        # Check if the point (x, y) is within the zone's boundaries
        return (self.start_x <= x <= self.end_x or self.end_x <= x <= self.start_x) and \
               (self.start_y <= y <= self.end_y or self.end_y <= y <= self.start_y)

    def check_zone_entry_exit(self, x, y, screen, font):
        # Determine if the user enters or exits the zone
        in_zone = self.is_in_zone(x, y)
        ...
        if in_zone and not self.is_user_in_zone:
            ...
            self.violations += 1
            self.is_user_in_zone = True
            self.display_time = time.time()  # Record the time of entry
            self.display_text(screen, font, f"Entering Zone {self.id}")

        elif not in_zone and self.is_user_in_zone:
            # User left the zone
            self.is_user_in_zone = False
            self.display_time = time.time()  # Record the time of exit
            self.display_text(screen, font, f"Leaving Zone {self.id}")

    def display_text(self, screen, font, message):
        # Display text on the screen without filling the screen
        text = font.render(message, True, (255, 255, 255))  # White text
        screen.blit(text, (20, 20))  # Display the text at the top left of the screen

    def render_temporary_message(self, screen, font, duration=2):
        """Render the message for a limited duration."""
        if time.time() - self.display_time < duration:
            # Continue displaying the message if within the duration window
            message = f"In {self.name}" if self.is_user_in_zone else f"Left {self.name}"
...
def render_zone_messages(screen):
    font = pygame.font.Font(None, 36)
    for zone in zones:
        zone.render_temporary_message(screen, font)
