...
def save_zone_data(zone):
    """Save the data of a zone to the CSV file in the data/csv folder with the current date in the filename."""
    # Ensure the directory exists
    directory = 'data/csv'
    if not os.path.exists(directory):
        os.makedirs(directory)

    # Create file path for zone_data with current date
    current_date = datetime.datetime.now().strftime('%Y-%m-%d')
    file_path = os.path.join(directory, f'zones_data_{current_date}.csv')

    file_exists = os.path.isfile(file_path)
    with open(file_path, 'a', newline='') as csvfile:
        fieldnames = ['Zone ID', 'Zone Name', 'Total Time Spent (s)', 'Average Speed', 'Number of Violations', 'Violation Locations']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        if not file_exists:
            writer.writeheader()

        average_speed = zone.calculate_average_speed()
        writer.writerow({
            'Zone ID': zone.id,
            'Zone Name': zone.name,
            ...
            'Violation Locations': "; ".join([f"({x:.2f}, {y:.2f})" for x, y in zone.violation_locations])
        })
...
def update_player_position(player, screen, font, current_zone, zones, speed):
    ...
    if new_zone != current_zone:
        if current_zone:
            ...
            save_zone_data(current_zone)  # Save data for the previous zone
...
class TrialManager:
    ...
    def get_events_csv_file_path(self):
        ...
        return os.path.join(self.data_dir, f'violation_data_{date_str}.csv')
    ...
    def get_daily_csv_file_path(self):
        # Generate a file name based on the current date and selected weather preset name
        date_str = datetime.datetime.now().strftime('%Y-%m-%d')
        weather_name = self.world.selected_weather[1]
        return os.path.join(self.data_dir, f'interval_data_{date_str}_{weather_name}.csv')

    def get_totals_csv_file_path(self):
        date_str = datetime.datetime.now().strftime('%Y-%m-%d')
        return os.path.join(self.data_dir, f'overall_data_{date_str}.csv')
    ...
    def start_trial(self, player):
        ...
        # Get the current weather preset name from the world.
        self.weather_preset_name = self.world.selected_weather[1].replace(" ", "_")

        # Create the interval data CSV file now, using the selected weather name.
        self.csv_file_path = os.path.join(
            self.data_dir, 
            f'interval_data_{datetime.datetime.now().strftime("%Y-%m-%d")}_{self.weather_preset_name}.csv'
        )
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            # Update player position and zones, track the current zone
            current_zone = update_player_position(world.player, display, font, current_zone, zones, speed)
            ...
            trial_manager.render_timer(display)  # Render the timer during trial
            trial_manager.render_results(display)  # Show trial results if the trial ends
