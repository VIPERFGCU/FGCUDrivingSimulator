## This script manages a CARLA simulation trial, tracks user behavior, and saves data to four CSV files:

## 1. violation_data_<date>.csv
##    - Logs speed violations including session/trial codes, timestamps, and the player's location.
##    - Tracks when and where speed limits were exceeded.

## 2. zones_data_<date>.csv
##    - Records time spent, average speed, and violations in each zone.
##    - Useful for analyzing behavior in different map areas.

## 3. interval_data_<date>_<weather>.csv
##    - Logs interval-based data like speed, location, and timestamps.
##    - Filename includes weather condition (e.g., 'ClearNoon') to match trial settings.

## 4. overall_data_<date>.csv
##    - Summarizes trial performance, including total time, max/min speeds, and violation counts.
##    - Includes a field for weather conditions used in the trial.
...
# Zone Class for tracking violations and storing data
class Zone:
    def __init__(self, name, start_x, start_y, end_x, end_y, id, speed_limit=45.0, debounce_time=1.5):
        self.name = name
        ...
        self.id = id
        self.violations = 0
        self.violation_locations = []  # List to store the x, y coordinates of violations
        self.is_user_in_zone = False
        self.last_trigger_time = 0  # Time when the last entry/exit event occurred
        self.debounce_time = debounce_time  # Minimum time between zone transitions
        self.entry_time = None
        self.total_time_spent = 0.0
        self.total_speed = 0.0
        self.speed_measurements = 0
        self.speed_limit = speed_limit
        self.entry_count = 0
        self.currently_speeding = False  # Track whether the player is currently speeding in the zone

    def is_in_zone(self, x, y, margin=1.0):
        """Check if the player is inside the zone, with a margin."""
        return (self.start_x - margin <= x <= self.end_x + margin or self.end_x - margin <= x <= self.start_x + margin) and \
               (self.start_y - margin <= y <= self.end_y + margin or self.end_y - margin <= y <= self.start_y + margin)

    def enter_zone(self):
        """Called when the user enters the zone."""
        self.entry_time = time.time()
        self.entry_count += 1
        self.currently_speeding = False  # Reset speeding state upon entering a new zone

    def leave_zone(self):
        """Called when the user leaves the zone."""
        if self.entry_time:
            self.total_time_spent += time.time() - self.entry_time
            self.entry_time = None
        self.currently_speeding = False  # Reset speeding state upon leaving the zone

    def update_speed(self, speed, x, y):
        """Update the total speed value and count speed violations."""
        if self.entry_time:
            self.total_speed += speed
            self.speed_measurements += 1
            if speed > self.speed_limit:
                if not self.currently_speeding:
                    # Count violation only once when speeding starts
                    self.violations += 1
                    self.violation_locations.append((x, y))  # Store the violation location
                    self.currently_speeding = True
            else:
                # Reset speeding state if speed goes below the limit
                self.currently_speeding = False

    def calculate_average_speed(self):
        """Calculate the average speed for the zone."""
        return self.total_speed / self.speed_measurements if self.speed_measurements > 0 else 0

# Function to check the current zone of the player
def check_user_position(x, y, current_zone, zones):
    """Check the player's position and update the zone if they move into a new one."""
    for zone in zones:
        if zone.is_in_zone(x, y) and zone != current_zone:
            # User entered a new zone
            return zone  # Return the new zone

    return current_zone  # No change in zone

# Function to display the current zone on the screen
def display_current_zone(screen, font, current_zone):
    """Display the current zone name on the screen."""
    if current_zone:
        message = f"Zone {current_zone.id}: {current_zone.name}"
        text = font.render(message, True, (0, 0, 0))  # Black text
        screen.blit(text, (screen.get_width() // 2 - text.get_width() // 2, 20))  # Centered at the top

# Function to save the zone data
def save_zone_data(zone, user_folder, trial_manager):
    """Save zone data with trial numbering."""
    if user_folder is None:
        print("Warning: user_folder is None, skipping zone data save.")
        return  

    date_str = datetime.datetime.now().strftime('%Y-%m-%d')
    weather_name = trial_manager.weather_preset_name.replace(" ", "_")
    trial_number = trial_manager.current_trial_number  # Use the stored trial number

    if trial_number is None:
        print("Max trials reached. No new trial will be recorded.")
        return  # Stop if all trials are complete
    trial_manager.current_trial_number = trial_number  # Store it for consistency

    file_path = os.path.join(user_folder, f'zones_data_{trial_number}_{date_str}.csv')

    file_exists = os.path.isfile(file_path)
    with open(file_path, 'a', newline='') as csvfile:
        fieldnames = ['Zone ID', 'Zone Name', 'Total Time Spent (s)', 'Average Speed', 'Number of Violations']
        ...
        writer.writerow({
            'Zone ID': zone.id,
            'Zone Name': zone.name,
            'Total Time Spent (s)': round(zone.total_time_spent, 2),
            'Average Speed': round(zone.calculate_average_speed(), 2),
            'Number of Violations': zone.violations
        })


# Initialize zones
zones = [
    Zone("Bottom Right Curve Zone (Start)", 244.44, -16.59, 164.31, -205.97, 1),
    Zone("Middle Right Straight Zone", 161.33, -206.10, -8.43, -206.65, 2),
    Zone("Top Right Curve Zone", -10.96, -206.53, -86.73, -148.57, 3),
    Zone("Top Middle Straight Zone", -86.85, -146.21, -87.03, 142.16, 4),
    Zone("Top Left Curve Zone", -86.75, 144.36, -11.00, 205.85, 5),
    Zone("Middle Left Straight Zone", -3.50, 205.95, 147.17, 205.27, 6),
    Zone("Bottom Left Curve Zone", 154.32, 205.46, 240.54, 137.83, 7),
    Zone("Bottom Middle Straight Zone (Finish)", 240.70, 130.50, 244.36, -11.00, 8)
]

# Function to update player position
def update_player_position(player, screen, font, current_zone, zones, speed, user_folder, trial_manager):
    """Update player position and manage zone transitions."""
    # Get player's current coordinates using get_transform
    transform = player.get_transform()
    x, y = transform.location.x, transform.location.y

    # Update the current zone based on the player's position
    new_zone = check_user_position(x, y, current_zone, zones)

    # If the player entered a new zone, manage transition
    if new_zone != current_zone:
        if current_zone:
            current_zone.leave_zone()  # Player leaves current zone
            save_zone_data(current_zone, user_folder, trial_manager)

        if new_zone:
            new_zone.enter_zone()  # Player enters the new zone

    # Update speed in the current zone
    if new_zone:
        new_zone.update_speed(speed, x, y)

    # Render the current zone message
    display_current_zone(screen, font, new_zone)

    return new_zone  # Return the updated zone
...
class World(object):
    def __init__(self, carla_world, hud, actor_filter, config_handler, trial_manager=None):
        ...
        self.selected_weather = self._weather_presets[self._weather_index]  # Store both (WeatherParameters, name)
    ...
    def next_weather(self, reverse=False):
        ...
        self.selected_weather = self._weather_presets[self._weather_index]  # Store (WeatherParameters, name)
        self.hud.notification(f'Weather: {self.selected_weather[1]}')  # Display the name
        self.world.set_weather(self.selected_weather[0])  # Apply the actual WeatherParameters
...
class TrialManager:
    def __init__(self, hud, config_handler, player, world):
        ...
        self.max_speed = self.trial_settings.get('speed_limit', 45.0)
        self.trial_results = {}
        ...
        self.selected_name_index = len(self.user_names) - 1
        if self.selected_name_index >= 0:
            self.user_name = self.user_names[self.selected_name_index]

        # Initialize data_to_write for storing interval data before writing to CSV
        self.data_to_write = []
    ...
    def create_user_folder(self, user_name):
        """Create or reuse a folder named with the user's name."""
        folder_name = f"{user_name}"  # No date in the folder name
        folder_path = os.path.join('data', 'csv', folder_name)
        os.makedirs(folder_path, exist_ok=True)
        return folder_path

    def get_trial_number(self):
        """Ensure only three trials per user folder, corresponding to different weather conditions."""
        existing_files = os.listdir(self.user_folder)
        trial_numbers = set()

        for file in existing_files:
            match = re.search(r'_(\d+)_\d{4}-\d{2}-\d{2}\.csv', file)
            if match:
                trial_numbers.add(int(match.group(1)))

        # Cap the number of trials at 8 per user
        if len(trial_numbers) >= 8:
            print("All 8 trials are already recorded. No new trials will be created.")
            return None

        next_trial = min(set(range(1, 9)) - trial_numbers, default=None)
        return f"{next_trial:02d}" if next_trial else None

    def get_events_csv_file_path(self):
        """Generate the violation data CSV file path with the current date and trial number."""
        date_str = datetime.datetime.now().strftime('%Y-%m-%d')
        trial_number = self.current_trial_number  # ✅ Use stored trial number

        return os.path.join(self.user_folder, f'violation_data_{trial_number}_{date_str}.csv')

    def init_events_csv(self):
        """Initialize the events CSV file with a header if it doesn't exist."""
        # Ensure the folder exists
        if not self.user_folder:
            raise ValueError("User folder not set. Ensure start_trial() is called first.")

        # Dynamically get the path when needed
        events_csv_path = self.get_events_csv_file_path()

        # Ensure the user folder exists and create the CSV file if necessary
        os.makedirs(self.user_folder, exist_ok=True)

        # Initialize the file only if it doesn't exist
        if not os.path.exists(events_csv_path):
            with open(events_csv_path, 'a', newline='') as events_file:
                events_writer = csv.writer(events_file)
                events_writer.writerow(['Session Code', 'Trial Code', 'Unix Timestamp', 'Event Type', 'Violation Duration', 'X', 'Y', 'Z'])
    
    def init_interval_csv(self):
        """Initialize the interval CSV file with a header if it doesn't exist."""
        if not self.user_folder:
            raise ValueError("User folder not set. Ensure start_trial() is called first.")

        # Generate the interval data CSV file path
        date_str = datetime.datetime.now().strftime('%Y-%m-%d')
        trial_number = self.get_trial_number()
        weather_name = self.weather_preset_name.replace(" ", "_")

        self.csv_file_path = os.path.join(self.user_folder, f'interval_data_{trial_number}_{date_str}_{weather_name}.csv')
        ...
        if not os.path.exists(self.csv_file_path):
            with open(self.csv_file_path, 'a', newline='') as interval_file:
                writer = csv.writer(interval_file)
                writer.writerow([
                    'User Name', 'Session Code', 'Trial Code', 'Unix Timestamp',
                    'Time Elapsed', 'Speed', 'X', 'Y', 'Z', 'Vehicle'
                ])
    ...
    def get_daily_csv_file_path(self):
        """Generate the interval data CSV file path with the current date, weather, and trial number."""
        date_str = datetime.datetime.now().strftime('%Y-%m-%d')
        weather_name = self.weather_preset_name.replace(" ", "_")  # ✅ Make sure weather name exists
        trial_number = self.current_trial_number  # ✅ Use the stored trial number

        return os.path.join(self.user_folder, f'interval_data_{trial_number}_{date_str}_{weather_name}.csv')

    def get_totals_csv_file_path(self):
        """Generate the overall data CSV file path with the current date and trial number."""
        date_str = datetime.datetime.now().strftime('%Y-%m-%d')
        trial_number = self.current_trial_number  # ✅ Use the stored trial number

        return os.path.join(self.user_folder, f'overall_data_{trial_number}_{date_str}.csv')
    ...
    def start_trial(self, player):
        ...
        self.user_folder = self.create_user_folder(user_name) if user_name else os.path.join('data', 'csv', 'default_user')
        os.makedirs(self.user_folder, exist_ok=True)

        self.user_name = user_name

        # 🔹 **Fix: Set the current trial number before using it**
        self.current_trial_number = self.get_trial_number()  

        # 🔹 **Fix: Set weather preset name before initializing interval CSV**
        self.weather_preset_name = self.world.selected_weather[1].replace(" ", "_")

        self.csv_file_path = self.get_daily_csv_file_path()  # ✅ No extra argument
        self.events_csv_file_path = self.get_events_csv_file_path()  # ✅ No extra argument
        self.totals_csv_file_path = self.get_totals_csv_file_path()  # ✅ No extra argument

        self.init_interval_csv()
        self.init_events_csv()

        # Teleport the player to the start location and rotation
        teleport_location = carla.Location(
            x=self.trial_settings.get('location_x', 243.98), 
            y=self.trial_settings.get('location_y', -3.39), 
    ...
    def write_totals_to_csv(self):
        ...
        with open(totals_csv_path, 'a', newline='') as totals_file:
            ...
            totals_writer.writerow([
                ...
                round(self.trial_results['avg_speed'], 2),
                round(self.max_speed_during_run, 2),
                round(self.min_speed_during_run, 2),
                self.trial_results['violation_count'],
                f"{self.trial_results['avg_violation_duration']:.2f} seconds",
                self.vehicle_type,
                self.weather_preset_name  # Include the weather preset in the overall data
            ])
    ...
    def track_speed(self, speed, player):
        ...
        if self.trial_active:
            # Track the maximum and minimum speed during the trial
            ...
            # Check if the player is exceeding the speed limit
            if speed > 50:
                # If violation has not started, start it and increment violation count
                if self.violation_start is None:
                    self.violation_start = current_time
                    self.violation_count += 1
                self.speeding_warning = True
            else:
                # Speed is below the limit, end any active violation
                if self.violation_start is not None:
                    violation_end = current_time
            ...
            if self.start_time is not None and current_time - self.start_time >= len(self.data_to_write) * 0.5:
                location = player.get_transform().location
                rotation = player.get_transform().rotation
                ...
                self.data_to_write.append([
                    ...
                    round(current_time - self.start_time, 2), speed,
                    location.x, location.y, location.z, self.vehicle_type
                ])

        else:
            # End any ongoing violation when the trial ends
            if self.violation_start is not None:
                violation_end = current_time
                violation_duration = violation_end - self.violation_start
                location = player.get_transform().location
        ...
        # Ensure warning is cleared if no trial is active
    ...
    def render_timer(self, display):
        ...
        # Speed meter is always displayed, even when the trial is not active
        speed_text = f"Speed: {int(round(self.current_speed))} mph"
...
def game_loop(args):
    ...
    world = None  # To store the game world object
    fullscreen = False  # Flag to track if fullscreen mode is enabled
    display = pygame.display.set_mode((1280, 720))
    font = pygame.font.Font(None, 36)  # Font for displaying zone text
    current_zone = None  # Initialize the current zone as None
    ...
    try:
        ...
        # Set up display with Pygame
        display = pygame.display.set_mode(
            (args.width, args.height),
            pygame.HWSURFACE | pygame.DOUBLEBUF)

        # Load the specified town map in CARLA
        ...
        # Create HUD and world first
        hud = HUD(args.width, args.height, config_handler)
        world = World(client.get_world(), hud, args.filter, config_handler)  # Create the World instance

        # Now create the TrialManager using the world.player
        trial_manager = TrialManager(hud, config_handler, world.player, world)  # Pass the world instance to the trial manager

        # Assign the trial_manager to the World instance
        world.trial_manager = trial_manager

        # Initialize the control system (keyboard, joystick, etc.)
        ...
        while True:
            ...
            # Get player's speed (velocity to mph)
            v = world.player.get_velocity()
            speed = 2.23694 * math.sqrt(v.x**2 + v.y**2 + v.z**2)  # Convert m/s to mph
            trial_manager.track_speed(speed, world.player)  # Call this every frame in the game loop


            if trial_manager.user_folder is not None:  # Ensure folder exists before updating position
                current_zone = update_player_position(world.player, display, font, current_zone, zones, speed, trial_manager.user_folder, trial_manager)

            # Update the display with Pygame
            pygame.display.flip()

            # Parse input events (keyboard, joystick, etc.)
            if controller.parse_events(world, clock, trial_manager):
                return  # If the user wants to quit, break the loop

            # Handle trial start and initiation
            keys = pygame.key.get_pressed()  # Get current keyboard state
            if keys[pygame.K_1]:  # Start a new trial with key '1'
                trial_manager.start_trial(world.player)
            if keys[pygame.K_RETURN] and trial_manager.start_screen:  # Start trial if at start screen
                trial_manager.initiate_trial()
            if keys[pygame.K_SPACE] and trial_manager.trial_active:  # End trial with space key
                trial_manager.end_trial()
            if keys[pygame.K_RETURN] and trial_manager.display_results:  # Restart trial after showing results
                trial_manager.start_trial(world.player)

            # Handle the "F" key to toggle fullscreen
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_f:  # Press 'F' to toggle fullscreen
                        ...
                        if fullscreen:
                            display = pygame.display.set_mode(
                                (args.width, args.height), pygame.HWSURFACE | pygame.DOUBLEBUF | pygame.FULLSCREEN)
                        else:
                            display = pygame.display.set_mode(
            ...
            # Update the game world and render everything to the display
            world.tick(clock)
            world.render(display)
            trial_manager.render_timer(display)  # Always display the speed meter

            trial_manager.render_start_screen(display)  # Render the start screen if needed
            trial_manager.render_results(display)  # Show trial results when the trial ends

            # Update the Pygame display
            pygame.display.flip()

    finally:
        # Destroy the world and quit Pygame when exiting the loop
...
if __name__ == '__main__':
    main()
class Zone:
    def __init__(self, name, start_x, start_y, end_x, end_y, id, debounce_time=1.5):
        self.name = name
        self.start_x = start_x
        self.start_y = start_y
        self.end_x = end_x
        ...
        self.is_user_in_zone = False
        self.last_trigger_time = 0  # Time when the last entry/exit event occurred
        self.display_time = 0  # Time when the message was triggered
        self.debounce_time = debounce_time  # Minimum time between zone messages
        self.user_folder = os.path.join('data', 'csv', 'default_user')
        os.makedirs(self.user_folder, exist_ok=True)
    ...
    def record_violation(self):
        self.violations += 1
        print(f"User entered {self.name}! Violations recorded: {self.violations}")
...
def check_user_position(x, y, screen, font, current_zone, zones):
    """Check the player's position and update the zone if they move into a new one."""
    for zone in zones:
        if zone.is_in_zone(x, y) and zone != current_zone:
            # User entered a new zone
